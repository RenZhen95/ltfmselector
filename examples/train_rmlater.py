...
if len(sys.argv) < 8:
    print(
        "Possible usage: python3 train.py <dataset> <medscores> " +
...
AgentSelector = LTFMSelector(
    ...
    fQueryCost=query_cost, mQueryCost=query_cost,
...
doc, ActionValuesQ = AgentSelector.fit(
    ...
    lr=LR, monitor=True, returnQ=True, smsproject=True
)
...
savePklTupleFileName = f"{saveFileName}_nE{nEpisodes}_ActionValuesQ.pkl_tuple"
with open(savePklTupleFileName, "wb") as handle:
    pickle.dump(ActionValuesQ, handle)
